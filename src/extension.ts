...
export async function showInputBox() {
	const result = await vscode.window.showInputBox({
		...
		title: 'Scribe AI',
		prompt:
			'You have not set your OpenAI API key yet or your API key is incorrect, please enter your API key to use the ScribeAI extension.',
...
export async function activate(context: vscode.ExtensionContext) {
	...
	if (
		vscode.workspace.getConfiguration('scribeai').get('ApiKey') === '' ||
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('mywiki.createNote', (reply: vscode.CommentReply) => {
			replyNote(reply);
		})
	);

	context.subscriptions.push(
		vscode.commands.registerCommand('mywiki.askAI', (reply: vscode.CommentReply) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('mywiki.aiEdit', (reply: vscode.CommentReply) => {
			vscode.window.withProgress(
				{
					location: vscode.ProgressLocation.Notification,
					title: 'Generating AI response...',
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('mywiki.genDocString', (reply: vscode.CommentReply) => {
			vscode.window.withProgress(
				{
					location: vscode.ProgressLocation.Notification,
					title: 'Generating AI response...',
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('mywiki.replyNote', (reply: vscode.CommentReply) => {
			replyNote(reply);
		})
	);

	context.subscriptions.push(
		vscode.commands.registerCommand('mywiki.deleteNoteComment', (comment: NoteComment) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('mywiki.deleteNote', (thread: vscode.CommentThread) => {
			thread.dispose();
		})
	);
	...
	async function generatePromptChatGPT(question: string, thread: vscode.CommentThread) {
		...
		for (
			...
			if (filteredComments[i].author.name === 'VS Code') {
				messages.push({
					role: 'user',
					content: `${(filteredComments[i].body as vscode.MarkdownString).value}`,
				});
			} else if (filteredComments[i].author.name === 'Scribe AI') {
				messages.push({
					role: 'assistant',
